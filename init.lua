...
--spec "user.luarocks" --is required for solving dependencies for plugins, didn't work well for me and gave a lot of errors  
spec "user.colorscheme" -- different colorschemes can be loaded by uncommenting or commenting them by deleting --[[ and ]]-- at the beginning and at the end (double hyphen and square brackets)  
...
spec "user.mason" --language server, was working fine and can be activated any time but high cpu usage and turns on the fan. Since I do not really need it at the moment I deactivated it.  
--spec "user.schemastore"
--spec "user.lspconfig" --language server configuration; keymap which collisioned with :nextbuffer and Shift+K - can be reactivated but needed lot of cpu power.  
--spec "user.navic"
...
spec "user.nvimtree" -- A File Explorer For Neovim Written In Lua; another option is Fern, press "Space+e" to toggle the function.  
...
--spec "user.lualine" --Shows in the status line at the bottom in which mode you are, by color. Insert, Normal, Replace. From chrismachine config. Similar to bufferline or lualine-nvim.  
--spec "user.floatpreview" --giving a preview of the file which is selected in the sidebar with nvimtree. So you can see the content of the file in a small floating window. Not sure if needed in init.lua. Couln't make it work.  
spec "user.lualine-nvim" -- Shows in the status line at the bottom or top in which mode you are, by color. Insert, Normal, Replace. From Theena from Oviwrite. Similar to bufferline.
...
spec "user.snack"  --was required after some update gave me an error message.  
